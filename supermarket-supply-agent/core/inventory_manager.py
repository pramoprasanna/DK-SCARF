...
def identify_low_stock(df):
    ...
    grouped = df.groupby(['ItemID', 'ItemName', 'Item Size'])['StockQty'].sum().reset_index()
...
def detect_expiring_batches(df):
    ...
    print("Displaying items expiring / expired by ", upcoming)
    expiry_products = df[df['ExpiryDateBatch'] <= upcoming]
    return expiry_products[['ItemID', 'ItemName', 'Item Size', 'BatchID', 'ExpiryDateBatch', 'StockQty']]
